...
void RadioSend(uint8_t *buffer, uint8_t size, uint32_t timeout)
{
	...
	// Duration (s) = Period * 0.000015625 (see DS_SX1261-2_V2.1 datasheet chapter 4.1.1)
	...
	//
	// Example if duration = 2ms => period = 2 << 64 = 128 and 128 * 0.015625 = 2ms
...
void RadioRx(uint32_t timeout)
{
	...
	if (RX_CONTINUOUS_MODE == timeout)
	{
		RxContinuous = true;
		LOG_LIB("RADIO", "RX continuous mode");
	}
	else
	{
		RxContinuous = false;

		if (RX_SINGLE_MODE == timeout)
		{
			LOG_LIB("RADIO", "RX single mode");
		}
		else
		{
			LOG_LIB("RADIO", "RX window timeout = %ld", timeout);
			// Duration (s) = Period * 0.000015625 (see DS_SX1261-2_V2.1 datasheet chapter 13.1.4)
			// Period = Duration (s) / 0.000015625
			// 1/0.000015625 => 64000 => SX126X RTC 64 kHz clock (see DS_SX1261-2_V2.1 datasheet chapter 4.1.1)
			// Period = Duration (s) * 64000
			// Period = Duration (ms) * 64000 / 1000 = Duration (ms) * 64 = Duration (ms) << 6
...
void IRAM_ATTR RadioOnDioIrq(void)
{
	...
	IrqFired = true;
	BoardEnableIrq();
	// Wake up LoRa event handler on ESP32
...
void RadioBgIrqProcess(void)
{
	...
	if (IrqFired == true)
	{
		...
		if ((irqRegs & IRQ_TX_DONE) == IRQ_TX_DONE)
		{
			...
			LOG_LIB("RADIO", "RadioIrqProcess => IRQ_TX_DONE");
		...
		if ((irqRegs & IRQ_RX_DONE) == IRQ_RX_DONE)
		{
			...
			LOG_LIB("RADIO", "RadioIrqProcess => IRQ_RX_DONE");
		...
		{
			LOG_LIB("RADIO", "RadioIrqProcess => IRQ_CAD_DONE");
		...
		if ((irqRegs & IRQ_RX_TX_TIMEOUT) == IRQ_RX_TX_TIMEOUT)
		{
			...
			{
				LOG_LIB("RADIO", "RadioIrqProcess => IRQ_TX_TIMEOUT");
				//!< Update operating mode state to a value lower than \ref MODE_STDBY_XOSC
				SX126xSetOperatingMode(MODE_STDBY_RC);
			...
			{
				LOG_LIB("RADIO", "RadioIrqProcess => IRQ_RX_TIMEOUT");
				//!< Update operating mode state to a value lower than \ref MODE_STDBY_XOSC
				SX126xSetOperatingMode(MODE_STDBY_RC);
		...
		{
			LOG_LIB("RADIO", "RadioIrqProcess => IRQ_PREAMBLE_DETECTED");
		...
		{
			LOG_LIB("RADIO", "RadioIrqProcess => IRQ_SYNCWORD_VALID");
			//__NOP( );
		}

		if ((irqRegs & IRQ_HEADER_VALID) == IRQ_HEADER_VALID)
		{
			LOG_LIB("RADIO", "RadioIrqProcess => IRQ_HEADER_VALID");
			//__NOP( );
		}
